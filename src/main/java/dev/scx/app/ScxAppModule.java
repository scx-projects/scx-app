...
///
/// 1. ScxApp 调用所有模块的 `define()`
///
///    每个模块返回自身的 ScxAppModuleDefinition.
///
...
/// - SQL 模块的 start 可以是根据 数据源配置创建 SQLClient;
...
/// 用 define / start / stop 保持模块 API 稳定.
/// ```
///
/// @author scx567888
public interface ScxAppModule {

    /// 返回当前模块在本次 ScxApp 中的定义.
    ///
    /// ScxApp 会收集所有模块返回的 ScxAppModuleDefinition,
    /// 并在之后统一构建模块启动顺序.
    ///
    /// @return 当前模块的定义信息
    default ScxAppModuleDefinition define() throws Exception {
        return ScxAppModuleDefinition.of();
    }

    /// 执行当前模块的启动动作.
    ///
    /// 该方法会在所有模块 define() 完成之后调用.
    /// ScxApp 会根据各模块定义中的 startBefore / startAfter 计算调用顺序.
    ///
    /// @param scxApp 当前 ScxApp 实例
    default void start(ScxApp scxApp) throws Exception {
    ...
    ///
    /// @param scxApp 当前 ScxApp 实例
    default void stop(ScxApp scxApp) throws Exception {
